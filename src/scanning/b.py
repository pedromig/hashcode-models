...
class Solution:
    def __init__(self: Solution, problem: Problem, books: Optional[list[set[int]]] = None,
                 ...
                 libraries: Optional[set[int]] = None, closed: Optional[set[int]] = None,
                 open: Optional[int] = None, day: Optional[int] = None, 
                 start: Optional[list[int]] = None, objv: Optional[int] = None,
        ...
        self.open = open if open is not None else None # Current Library
    ...
    def copy(self: Solution) -> Solution:
        return Solution(self.problem, copy.deepcopy(self.books), self.quota.copy(),
                        copy.deepcopy(self.used), self.libraries.copy(), self.closed.copy(),
                        self.open, self.day, self.start.copy(), self.objv, self.ub, self.ub_kp.copy(),
    ...
    def add_moves(self: Solution) -> Iterable[Component]:
        if self.__signed(self.open) and self.__has_quota(self.open):
            for book in self.problem.books[self.open]:
                if book not in self.books[self.open] and not self.__forbidden(book, self.open):
    ...
    def add(self: Solution, c: Component) -> None: 
        ...
        ub_library, ub_day = self.open, self.day
        ...
        # Update Bound
        self.ub = self.__upper_bound_update_add(c, ub_library, ub_day, self.ub_kp, self.ub_lim, self.ub_full)
    ...
    def upper_bound_increment_add(self: Solution, c: Component) -> int:  
        ub = self.__upper_bound_update_add(c, self.open, self.day, self.ub_kp.copy(), self.ub_lim.copy(), self.ub_full.copy())
        return ub - self.ub
    ...
    def __remove(self: Solution, c: Component) -> int:
        ...
        if c.book is None: 
            ...
            if self.open != c.library:
                self.closed.remove(self.open)
    ...
    def __upper_bound_update_add(self: Solution, c: Component, close: int,
                                 day: int, ub_kp: list[int], ub_lim: list[int], 
                                 ub_full: list[int]) -> int:
        ub = 0
        if c.book is None:
            if close is not None: 
                ub_kp[close] -= (self.problem.rate[close] * (self.problem.d - day)) - len(self.books[close])
                while ub_kp[close] < 0 and ub_lim[close] > 0:
                    ub_lim[close] -= 1
                    book = self.problem.sbooks[close][ub_lim[close]]
                    if not self.__forbidden(book, close):
                        ub_kp[close] += 1
                        ub_full[close] -= self.problem.scores[book]

            d = self.problem.signup[c.library]
            for library in range(self.problem.l):
                if not self.__signed(library) and library != c.library and library != close:
