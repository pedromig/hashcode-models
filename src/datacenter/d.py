...
# FIXME: check best fit and first fit
# FIXME: check forbid components that don't fit anywhere
...
class Solution:
    ...
    def add_moves(self: Solution) -> Iterable[Component]:
        if self.iserver < self.problem.m:
            ...
            if server in self.unused:
                if segment := self.__best_fit(server):
    ...
    def add(self: Solution, c: Component) -> None:
        if c.pool is None or c.segment is None:
            ...
            # Forbid segments that don't fit anywhere
            # remove = set()
            # for server in self.unused:
            #   size, _ = self.problem.servers[server]
            #   if all(space < size for space in self.ss):
            #     remove.add(server)
            
            # for server in remove: 
            #   self.__forbid(Component(self.problem, server))
            #   self.__upper_bound_update_forbid(c, self.ub_kp, self.ub_lim, self.ub_full, self.ub_frac)
    ...
    def __first_fit(self: Solution, server: int) -> int:
        for row in range(self.problem.r):
            for segment in self.problem.rows[row]:
                if self.__fits(server, segment):
                    return segment
     
    def __best_fit(self: Solution, server: int) -> int:
        best, size = None, None
        for row in range(self.problem.r):
            for segment in self.problem.rows[row]:
                if self.__fits(server, segment):
                    if best is None or self.ss[segment] < size:
                        best = segment 
                        size = self.ss[segment]
        return best
